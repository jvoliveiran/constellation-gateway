...
export const envSchema = z.object({
  ...
  PUBLIC_OPERATIONS: z.string().optional().default(''),
  ...
  SUBGRAPH_TIMEOUT_MS: z.coerce.number().min(1000).default(30000),
  SUBGRAPH_RETRY_COUNT: z.coerce.number().min(0).max(5).default(2),
  SUBGRAPH_RETRY_DELAY_MS: z.coerce.number().min(50).default(200),
  CIRCUIT_BREAKER_THRESHOLD: z.coerce.number().min(1).default(5),
  CIRCUIT_BREAKER_RESET_MS: z.coerce.number().min(1000).default(30000),
  APQ_ENABLED: z.coerce.boolean().default(true),
  RESPONSE_CACHE_ENABLED: z.coerce.boolean().default(false),
  RESPONSE_CACHE_TTL: z.coerce.number().min(1).default(60),
  RESPONSE_CACHE_MAX_SIZE: z.coerce.number().min(1).default(500),
  TOKEN_REVOCATION_ENABLED: z.coerce.boolean().default(false),
